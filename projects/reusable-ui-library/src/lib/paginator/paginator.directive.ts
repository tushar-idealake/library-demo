...
import { Page, PaginatorInstance } from './paginator.interface';
@Directive({
  selector: 'Paginator,[Paginator]',
  exportAs: 'Paginator',
})
export class PaginatorDirective implements OnInit, OnDestroy {
  /**
   ...
   */
  @Input() id: string = 'PAGINATOR_DEFAULT';
  ...
  /**
   *
   * @param PaginatorService serivce for angular paginator
   */
  constructor(private PaginatorService: PaginatorService) {
    // subscribe to changes
    this.subscription = this.PaginatorService.change.subscribe((id: string) => {
      if (id == this.id) {
        this.updatePages();
      }
    });
  ...
  private updatePages(): void {
    const instance: PaginatorInstance = this.PaginatorService.getInstance(this.id);
   ...
   */
  private outOfBoundCorrection(instance: PaginatorInstance): number {
   ...
   */
  isValidId(): void {
    if (!this.PaginatorService.getInstance(this.id)) {
