...
const isArrowUpDown = (keyCode: number) =>
  ...
  implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor
{
  ...
  @Input() public initialValue!: any & never;
  /**
  ...
  private defaultWriteValue(value: any & never) {
   ...
   * @param value
   */
  public setInitialValue(value: any & never) {
  ...
  public select(item: never & any) {
    ...
    if (this.initialValue) {
      ...
      if (history) {
        ...
        if (
          !existingHistory.some((existingItem: any & never) =>
          ...
          if (!this.isTypeString(item)) {
            ...
            const selectedIndex = copiedExistingHistory
              .map((el: { [x: string]: any }) => el[this.searchKeyword])
   ...
   * Define panel state
   */
  setPanelState(event: { stopPropagation: () => void }) {
    if (event) {
   ...
   * @param e event
   */
  removeHistoryItem(index: number, e: { stopPropagation: () => void }) {
    e.stopPropagation();
   ...
   * @param e event
   */
  resetHistoryList(e: { stopPropagation: () => void }) {
    e.stopPropagation();
    this.historyList = [];
    window.localStorage.removeItem(`${this.historyIdentifier}`);
    this.filterList();
